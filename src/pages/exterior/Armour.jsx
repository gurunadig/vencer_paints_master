import Navbar from '../../components/Navbar';
import Footer from '../../components/Footer';                      
import Getintouch from '../../components/Getintouch';
import InspireColorPallet from '../../components/InspireColorPallet';


function Armour(){
    return(

        <div>
            <Navbar/>
            
            <div class="row  container-fluid  m-0 p-0">
                   <img className="img-fluid" src="./assets/img/armour_banner.jpg" alt="exterior product armour"/>
                       <div className="fixbg_new">
                           <div className="row h-100 d-flex  align-items-start">
                             <div className="col-4"></div>
                             <div className="col-4 curved_interior"> 
                                  <div className=" def_ani wow animate__animated animate__fadeIn ">
                                    <h4 className="heading_interior text-black text_uppercase text-white mt-sm-5 ">Vencer Paints Armour</h4>
                                  </div>
                              </div>
                             <div className="col-4"></div>     
                           </div>
                       </div>
               </div>






 
    <section class="container-fluid bg_blue">
        <div class="container-lg">

            <div class="row">
                <div class="col-xs-4 col-md-4 mb-2 mt-5 pt-4 d-flex justify-content-center">
                    <img class="product_inner_img def_ani wow animate__animated animate__slideInLeft zoom_out" src="./assets/img/armour_exterior.png" alt="" />

                </div>
                <div class="col-xs-12 col-md-8 pt-4 pb-2 d-flex flex-column justify-content-center">
                    <h3 class="text-white text_uppercase text-center super_shield_h3 px-5 def_ani wow animate__animated animate__fadeInDown">vencer paints ARMOUR
                    </h3>
                    <p class="text-white pt-3 pl-2  text-center super_shield_p def_ani wow animate__animated animate__fadeIn">
                        Is high durability exterior paint to withstand conditions like heavy rainfall, extreme heat, high humidity. ARMOUR
                        fights fungus / ALGAE formation. Affording better coverage, Armour protects and beautifies walls, staying colour fast,
                        and season after season.</p>
                        
                    <p class="text-white pt-2 pl-2  text-center super_shield_p def_ani wow animate__animated animate__fadeIn">It is a special formulation based on pure acrylic resin emulsion and special pigments and
                        additives with high resistance to degradation by U.V radiation.</p>
                    <p class="text-white pt-2 pl-2  text-center super_shield_p def_ani wow animate__animated animate__fadeIn">It is resistant to alkali and other mild chemicals. The
                        product gives a tough coating with high adhesion on the exterior surface which is highly resistant to peeling. The
                        coating is highly resistant to all types of microbial attack thus preventing the onset of fungus and algae for years
                        together.</p>
                </div>
            </div>
        </div>
    </section>










    <div class="container-fluid bg_f5f5f5 pt-5">
        <div class="row mx-2">
            <div class="col-md-12">



                <ul class="nav nav-pills mb-3 row d-flex justify-content-around" id="pills-tab" role="tablist">
                    <li class=" col-md-3 nav-item" role="presentation">
                        <a class="nav-link active text_uppercase text-center custom_bg d-flex align-items-center justify-content-center  baseBlock_noHover def_ani wow animate__animated animate__fadeInDown"
                             id="pills-home-tab" data-toggle="pill"
                            href="#pills-home" role="tab" aria-controls="pills-home" aria-selected="true">Benefits</a>
                    </li>
                    <li class=" col-md-3 nav-item" role="presentation">
                        <a class="nav-link text_uppercase text-center custom_bg d-flex align-items-center justify-content-center baseBlock_noHover def_ani wow animate__animated animate__fadeInDown"
                            id="pills-profile-tab" data-toggle="pill" href="#pills-profile" role="tab"
                            aria-controls="pills-profile" aria-selected="false">Technical Data</a>
                    </li>
                    <li class=" col-md-3 nav-item" role="presentation">
                        <a class="nav-link text_uppercase text-center custom_bg d-flex align-items-center justify-content-center baseBlock_noHover def_ani wow animate__animated animate__fadeInDown"
                            id="pills-contact-tab" data-toggle="pill" href="#pills-contact" role="tab"
                            aria-controls="pills-contact" aria-selected="false">Usage</a>
                    </li>
                </ul>
                <div class="tab-content" id="pills-tabContent">

                    <div class="tab-pane fade show active " id="pills-home" role="tabpanel"
                        aria-labelledby="pills-home-tab">

                        <div class="row d-flex justify-content-md-between justify-content-center">


                            <div class="col-xs-12 col-sm-8 col-md-4  mt-5 d-flex justify-content-center">
                                <div
                                class="custom_img_box  bg_exc_work d-flex flex-column align-items-center justify-content-center card-2">
                                <img class="svg_inner_img" src="./assets/svg/Stain_Resistant.svg" alt=""/>
                                <p class="super_shield_p p-2">Protection from Exterior</p>
                            </div>
                            </div>

                            <div class="col-xs-12 col-sm-8 col-md-4 mt-5 d-flex justify-content-center">
                                <div
                                class="custom_img_box  bg_soft_sheen d-flex flex-column align-items-center justify-content-center card-2">
                                <img class="svg_inner_img" src="./assets/svg/Bright_finish.svg" alt=""/>
                                <p class="super_shield_p p-2">Bright Finish</p>
                            </div>
                            </div>

                            <div class="col-xs-12 col-sm-8 col-md-4 mt-5 d-flex justify-content-center">
                                <div
                                class="custom_img_box  bg_exc_work d-flex flex-column align-items-center justify-content-center card-2">
                                <img class="svg_inner_img" src="./assets/svg/water_based.svg" alt=""/>
                                <p class="super_shield_p p-2">Water Based</p>
                            </div>
                            </div>
                        </div>




                        <div class="row d-flex p-3 justify-content-md-center justify-content-center">

                            <div class="col-xs-12 col-sm-8 col-md-4 mt-5 d-flex justify-content-center">
                                <div
                                class="custom_img_box  bg_soft_sheen d-flex flex-column align-items-center justify-content-center card-2">
                                {/* <img class="svg_inner_img" src="./assets/svg/Emulsion_Finish.svg" alt=""/> */}
                                <p class="super_shield_p p-2">Emulsion Finish</p>
                            </div>
                            </div>

                            <div class="col-xs-12 col-sm-8 col-md-4 mt-5  mb-5  d-flex justify-content-center">
                                <div
                                class="custom_img_box  bg_exc_work d-flex flex-column align-items-center justify-content-center card-2">
                                <img class="" src="" alt=""/>
                                <p class="super_shield_p p-2">Exterior Surface</p>
                            </div>
                            </div>

                        </div>


                    </div>



                    <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">

                        <div class="row d-flex p-2 p-md-5 my-2 justify-content-center align-items-center">
                            
                            <div class="col-12 col-md-12 col-lg-11 card p-3 pt-4 baseBlock_noHover">
                                

                                <table class="card-body table table-bordered">
                                
                                    <tbody>
                                        <tr class="">
                                            <td class="table_bord  pl-1 pl-sm-5"><strong>Finish</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3">Medium Sheen</td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Paint Base Type</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3">Water Based Paint</td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Surface Drying Time</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3"></td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Coats Required</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3">2 coats required</td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Coverage</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3">60 -70 square feet / Litre for 2 coats</td>
                                        </tr>
                                        <tr class="">
                                            <td class=" table_bord pl-1 pl-sm-5"><strong>Dry for Recoating</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3">6 hours</td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Thinning</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3"></td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Specific Gravity</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3"></td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Packaging Size</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3">1LTR, 4LTR, 10LTR 	&#38; 20LTR</td>
                                        </tr>
                                        <tr class="">
                                            <td class="table_bord pl-1 pl-sm-5"><strong>Advantages</strong></td>
                                            <td class="table_bord pl-1 pl-sm-3"></td>
                                        </tr>
                                
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>


                    




                    <div class="tab-pane fade" id="pills-contact" role="tabpanel" aria-labelledby="pills-contact-tab">
                        <div class="row d-flex p-2 p-md-5 justify-content-center">
                            <div class="col-12 col-md-12 col-lg-11 p-2 p-md-5">
                                <h4 class="about_highlight_max text-left">Directions For Use </h4>
                                <ul class="p-3 ul_list_square">
                                    <li>
                                        <p class="tech_spec pl-2 py-1">Prepare wall surface by scraping off flaked
                                            coatings and ensure removal of fungus and algae growth with suitable biocide
                                            treatment.</p>
                                    </li>
                                    <li>
                                        <p class="tech_spec pl-2 py-1">The surface must be free from loose particles,
                                            dirt, dust, grease and moisture.</p>
                                    </li>
                                    <li>
                                        <p class="tech_spec pl-2 py-1">Take maximum precautions where the water tends to
                                            accumulate, i.e., places like parapet, sunshade, etc.</p>
                                    </li>
                                    <li>
                                        <p class="tech_spec pl-2 py-1">The exterior surface should be free from seepage
                                            and constant dampness. During the rainy reason allow 2-3 days of
                                            sunshine for surface to dry before starting painting.</p>
                                    </li>
                                    <li>
                                        <p class="tech_spec pl-2 py-1">Newly plastered walls must be allowed to cure for
                                            6 months before painting.</p>
                                    </li>
                                    <li>
                                        <p class="tech_spec pl-2 py-1">To 1 litre of <strong>ARMOUR</strong> add 400 ml of clean
                                            potable water. For best results, apply 2 coats. Between coats, allow
                                            minimum of 4 hours gap. On sensitive areas like parapets and awnings, an
                                            extra coat is recommended.</p>
                                    </li>
                                    <li>
                                        <p class="tech_spec pl-2 py-1">On a freshly plastered surface, 3 coats of Super
                                            Shield is necessary. Apply the first coat by diluting one litre of
                                            Super Shield with one litre of clean water. This coat acts as a primer coat.
                                            The other coats are to be applied by
                                            diluting with 400 ml of clean potable water per litre.</p>
                                    </li>
                                </ul>


                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

            <InspireColorPallet />
            <Getintouch />
            <Footer/>

        </div>
    )
}


export default Armour


